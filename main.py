...
class ProcessorFeatures(IntFlag):
    ...
    def _detect_x86(cls) -> 'ProcessorFeatures':
        ...
        if IS_WINDOWS:
            ...
            try:
                ...
                for flag, feat in [('sse', cls.SSE), ('sse2', cls.SSE2),
                                   ('sse3', cls.SSE3), ('ssse3', cls.SSSE3),
                                   ('sse4_1', cls.SSE41), ('sse4_2', cls.SSE42),
                                   ('avx', cls.AVX), ('avx2', cls.AVX2),
                                   ('fma', cls.FMA)]:
                    if flag in flags:
                        f |= feat
            except Exception:
    ...
    def names(self) -> List[str]:
        return [feat.name for feat in ProcessorFeatures if feat != ProcessorFeatures.BASIC and feat in self]
    ...
    def get_memory_info(self) -> Dict[str, Any]:
        try:
            class MEMSTAT(ctypes.Structure):
                _fields_ = [("length", ctypes.c_uint), ("memLoad", ctypes.c_uint),
